...
bool Student::insert() {
    QSqlQuery query;
    query.prepare("INSERT INTO `student` (`stuID`, `stuName`, `classID`, `gradeID`, `age`, `sex`, `nation`, `address`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
bool Student::selectById(int id) {
    QSqlQuery query;
    query.prepare(QString("SELECT * FROM %1 WHERE `stuID` = ?").arg(STDTOQSTR(this->tableName)));
    query.addBindValue(id);
    if (query.exec() && query.next()) {
        stuId = query.value("stuId").toString().toStdString();
...
bool Student::deleteData() {
    QSqlQuery query;
    query.prepare(QString("DELETE FROM %1 WHERE `stuID` =?").arg(STDTOQSTR(this->tableName)));
    query.addBindValue(STDTOQSTR(stuId));
    if(!query.exec()){
        return false; 
    }
...
bool Student::updateData() {
    QSqlQuery query;
    query.prepare(QString("UPDATE %1 SET `stuName` =?, `classID` =?, `gradeID` =?, `age` =?, `sex` =?, `nation` =?, `address` =? WHERE `stuID` =?").arg(STDTOQSTR(this->tableName)));
...
std::vector<DataObject*> Student::selectAll() {
    ...
    while (query.next()) {
        ...
        student->stuId = query.value("stuId").toString().toStdString();
